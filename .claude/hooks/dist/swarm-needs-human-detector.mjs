...
// src/swarm-needs-human-detector.ts
function getStatusboardPort() {
  try {
    const portsPath = existsSync("/app/ports.yaml") ? "/app/ports.yaml" : join(process.env.HOME || "", "local-ai", "ports.yaml");
...
var STATUSBOARD_URL = `http://host.docker.internal:${getStatusboardPort()}`;
